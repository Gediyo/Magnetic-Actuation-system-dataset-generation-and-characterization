...
//    //std::array<double, 3> EulerAngles = mainWindow->RotMat2EulerAngle(rotMat)
    ...
    Eigen::Affine3d T_OT = mainWindow->T_TO.inverse();
...
void Calib::on_RandDataCollect_clicked()
{
    bool ok;
    int num = ui->num->text().toInt(&ok); // number of randomized current values
    ...
    mainWindow->start_randData_collection(num);

}



void Calib::on_Grid_pt_toggled(bool checked)
{
    mainWindow->grid = checked;
    if(mainWindow->grid == true){
        std::cout<<"Grid on"<<std::endl;
    }
    if(mainWindow->grid == false){
