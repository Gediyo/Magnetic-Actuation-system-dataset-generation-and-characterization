...
//            // convert array of 16 into eigen matrix

//            Eigen::Map<Eigen::Matrix<double, 4, 4, Eigen::ColMajor>> matrix(robot_pose.data());
//            Eigen::Affine3d initial_transform(matrix);
//           // Eigen::Affine3d initial_transform(Eigen::Matrix4d::Map(robot_pose.data()));
//            Eigen::Vector3d r_O(initial_transform.translation());

//            current_EEpose = robot_pose;
//            sensor_position_T =  Cal_sensor_pos_T(r_O,current_EEpose); // calculate each sensor position realativ
...
//        }else{
        ...
        current_EEpose = robot_pose.O_T_EE;
        sensor_position_T =  Cal_sensor_pos_T(r_O,current_EEpose); // calculate each sensor position realative to the table

//        }

        MainWindow::Record_calibdata();

    }

    if (calc_run_temp_avg == 1){
        for (int i = 0; i<8; i++){
            if(i == 1 || i == 7) continue;
            run_avg_temp[i].push_back(measuredTemperatures[i]);
        }
//        if (!runnig_temp_timer->isActive()){
//            runnig_temp_timer->start(5000); // every 5 sec
//        }
